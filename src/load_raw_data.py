...
def main():
    ...
    log_message("INFO", f"*************************** START load_raw_data ***************************")
    log_message("INFO", f"load_raw_data {args.data_file_folder} {args.load_date} --create_table {args.create_table}")
    ...
    log_message("INFO", f"*************************** END load_raw_data ***************************")    
...
def process_table(table_name, folder, file, part, queries, create_table):
    ...
    if bool(create_table):
        log_message("INFO", f"Deleting table: {table_name}")
        rs_delete = execute_query(queries[0])
        log_message("INFO", f"Creating table: {table_name}")
        rs_create = execute_query(queries[1])
        log_message("INFO", f"Creating index on table: {table_name}")
        rs_index = execute_query(queries[2])
        if rs_delete and rs_create and rs_index:
            log_message("INFO", f"Loading data into table: {table_name}")
